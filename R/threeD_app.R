...
threeD_app = function(){
  ...
  server <- function(input, output, session) {
    ...
    output$regressPlot <- renderPlot({
      ...
      coefficients = MATH4773CLAG::coeff(input$xk1, input$xk2, spruce.df)


      curve(MATH4773CLAG::myf2(x,xk=input$xk1,xk2 = input$xk2, coef=coefficients),
            add=TRUE,
            lwd=2,
            col="Blue")
      ...
      points(input$xk1,MATH4773CLAG::myf2(input$xk1, input$xk1,input$xk2,coef=coef(lmp)),col="black",pch=21,bg="green",cex=2)

      points(input$xk2,MATH4773CLAG::myf2(input$xk2, input$xk1,input$xk2,coef=coef(lmp)),col="black",pch=21,bg="red",cex=2)
    ...
    output$R2 <- renderPlot({
      ...
      rsq_real = Vectorize(MATH4773CLAG::rsq_xk1_xk2)
      ...
      inter = MATH4773CLAG::matrix_rsquared(x1=input$intervalroot, x2=input$intervalroot2, h=0.1, data=spruce.df)
    ...
    output$root<-renderTable({
      intvs = MATH4773CLAG::matrix_rsquared(x1=input$intervalroot, x2=input$intervalroot2, h=0.1, data=spruce.df)
